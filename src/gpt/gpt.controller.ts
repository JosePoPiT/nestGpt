...
export class GptController {
      ...
      new ParseFilePipe({
        validators: [
          new MaxFileSizeValidator({
            maxSize: 1000 * 1024 * 5,
            message: 'File is bigger than 5 mb, you should use another one',
          }),
  ...
  async imageGeneration(@Body() imageGeneratioDto: ImageGenerationDto) {
  ...
  async getGeneratedImage(
    ...
    const filePath = this.gptService.imageGeneratedGetter(imageId);
    res.status(HttpStatus.OK);
    res.sendFile(filePath);
  }
